function deposit(id) {
    ...
    // Show deposit button for traditional payment methods (hidden for crypto)
    if(id != 7) {
        $("#deposit_form button[type='submit'], #deposit_form .register-btn").show();
    }
    ...
    if(id == 1) {
    ...
    } else if(id == 7) { // Crypto payment gateway - Solana
        amount = $("#crypto_amount").val();
        min_amount = $("#crypto_min_amount").val();
        max_amount = $("#crypto_max_amount").val();
        error_id = "#crypto_amount-error";

        // Hide all traditional banking fields for crypto (automatic processing)
        array_to_hide = [
            'mobile_div',
            'name_div', 
            'email_div',
            'utr_div',
            'trn_div',  // Hide transaction no. field - not needed for crypto
            'cwallet_div',
            'ctxt_div',
            'account_no_div',
            'account_holder_name_div',
        ...
        // Hide the manual deposit button - crypto is automatic
        $("#deposit_form button[type='submit'], #deposit_form .register-btn").hide();
    ...
    // Amount will be automatically handled by crypto system
