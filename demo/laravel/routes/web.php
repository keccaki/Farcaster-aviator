...
Route::post('/auth/magic-link', [Authentication::class, "sendMagicLink"]);
Route::get('/auth/magic-login', [Authentication::class, "magicLogin"]);
Route::get('/auth/magic-link/status', [Authentication::class, "checkMagicLinkStatus"]);
...
Route::group(['prefix' => 'admin/', 'middleware' => ['isAdmin']], function () {
    ...
    Route::get('/crypto-management', function () {
        return view('admin.crypto-management');
    });
...
Route::group(['middleware' => ['isUser']], function () {
    ...
    // Crypto wallet endpoints (simplified for testing)
    Route::prefix('api/crypto')->group(function() {
        Route::get('/wallet', function() {
            ...
            return response()->json([
                'success' => true,
                'wallet' => [
                    'address' => '9WzDXwBbmkg8ZTbNMqUxvQRAyrZzDsGYdLVL9zYtAWWM', // Dummy Solana address
                    'balance' => ['SOL' => '0.00', 'USDT' => '0.00']
                ]
            ]);
        });
        ...
        Route::post('/wallet/generate', function() {
            ...
            // For testing, generate a dummy wallet
            return response()->json([
                'success' => true,
                'wallet' => [
                    'address' => '9WzDXwBbmkg8ZTbNMqUxvQRAyrZzDsGYdLVL9zYtAWWM', // Dummy Solana address
        ...
        Route::post('/check-deposits', function() {
            if (!session()->has('userlogin')) {
                return response()->json(['success' => false, 'error' => 'Not authenticated'], 401);
            }
            
            // For testing, simulate no deposits found
            return response()->json([
                'success' => true,
...
Route::post('/telegram/initdata', [TelegramWebAppController::class, 'init'])->name('telegram.initdata');
Route::post('/telegram/results', [TelegramWebAppController::class, 'sendResult'])->name('telegram.results');

Route::view('/telegram-webapp', 'telegram-webapp')->name('telegram-webapp');
